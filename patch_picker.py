...
def save_coords(coords: list):
    with open(PATCH_COORDS_FILE, "w") as f:
        json.dump(coords, f, indent=2)
    print(f"Saved {len(coords)} patch coords to {PATCH_COORDS_FILE}")

def load_coords() -> list:
    if not PATCH_COORDS_FILE.exists():
        return []
    with open(PATCH_COORDS_FILE) as f:
        return json.load(f)

def apply_coords_to_year(coords: list):
    """Crop and save patches for the current year using saved coordinates."""
    for i, c in enumerate(coords, 1):
        lat_min, lat_max = c["lat_min"], c["lat_max"]
        lon_min, lon_max = c["lon_min"], c["lon_max"]
        ...
        if any(x is None for x in [crop_rgb, crop_ndsi, crop_qa]):
            print(f"  patch_{i:03d}: outside image extent, skipping.")
            continue

        crop_good = make_good_pixel_mask(crop_qa)
        px_w_m, px_h_m = meters_per_pixel_from_transform(prof)

        npz_path  = OUT_DIR / f"patch_{i:03d}_arrays.npz"
        meta_path = OUT_DIR / f"patch_{i:03d}_meta.txt"
        rgb_path  = OUT_DIR / f"patch_{i:03d}_rgb.png"
        ndsi_path = OUT_DIR / f"patch_{i:03d}_ndsi.png"
        ...
        print(f"  patch_{i:03d} saved → {npz_path.name}")
...
if existing:
    ans = input(f"Found {len(existing)} saved patch coords. Apply to {CURRENT_YEAR}? (y) or redefine (n): ").strip().lower()
    if ans == "y":
        print(f"\nApplying {len(existing)} saved patches to {CURRENT_YEAR}...")
        apply_coords_to_year(existing)
        print("Done.")
        exit()

# Otherwise fall through to interactive picker as normal
saved_coords = []
patch_id = 1
